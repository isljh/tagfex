...
class TagFexSimplifiedReplaceTAWithTask0TS(BaseLearner):
    def __init__(self, args):
        super().__init__(args)
        self._network = TagFexTask0TSReplaceNet(args, False, use_transfer_branch=False)
        self._global_step = 0
        ...
        if local_rank <= 0:
            swanlab.init(
                ...
                experiment_name="{}_{}_{}".format(
                    ...
                    self.args.get("model_name", "tagfex_simplified_replace_ta_with_task0_ts"),
                ),
                config=self.args,
                suffix="timestamp",
            )
