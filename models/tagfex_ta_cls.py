# TA-only incremental learner based on the task-agnostic branch.
import logging
import numpy as np
...
from utils.inc_net import TACLSNet
...
class TagFexTACls(BaseLearner):
    def __init__(self, args):
        super().__init__(args)
        self._network = TACLSNet(args, False)

        local_rank = self.args.get("local_rank", 0)
        if local_rank <= 0:
            swanlab.init(
                project="PyCIL_TagFex",
                experiment_name="TagFex_TA_CLS",
                config=self.args,
                suffix="timestamp"
            )
